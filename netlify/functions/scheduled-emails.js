// Runs every day at 7:00 AM UTC via Netlify Scheduled Functions
...
function reminderHtml(r) {
  return baseLayout(`
    ...
    <p style="color:#374151;margin-top:20px;">Nous vous enverrons les instructions d'accès la veille. À très bientôt !</p>`)
}
...
function welcomeHtml(r) {
  return baseLayout(`
    <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:14px;margin-bottom:20px;text-align:center;">
      <p style="margin:0;color:#16a34a;font-size:16px;font-weight:bold;">🎉 Bienvenue à la villa ${r.villa?.name} !</p>
    </div>
    <p style="color:#374151;">Bonjour <strong>${r.client?.full_name || 'Client'}</strong>,</p>
    <p style="color:#374151;">Nous vous souhaitons la bienvenue ! Voici vos informations d'accès :</p>
    <table style="width:100%;border-collapse:collapse;border:1px solid #e5e7eb;border-radius:8px;overflow:hidden;">
      ...
      <tr ${r.villa?.wifi_password ? '' : 'style="background:#f9fafb;"'}><td style="padding:10px 14px;color:#6b7280;font-size:14px;">📅 Départ</td><td style="padding:10px 14px;font-weight:600;text-align:right;">${fmt(r.check_out)}${r.check_out_time ? ' à ' + r.check_out_time : ''}</td></tr>
    </table>
    ...
    <p style="color:#374151;margin-top:20px;">Bon séjour ! N'hésitez pas à nous contacter pour toute question.</p>`)
}
...
function reviewHtml(r) {
  return baseLayout(`
    ...
    <p style="color:#374151;">Bonjour <strong>${r.client?.full_name || 'Client'}</strong>,</p>
    <p style="color:#374151;">Nous espérons que votre séjour à la villa <strong>${r.villa?.name}</strong> s'est très bien passé !</p>
    <p style="color:#374151;">Votre avis nous aide à améliorer nos services. Pourriez-vous prendre 2 minutes pour partager votre expérience ?</p>
    <div style="text-align:center;margin:24px 0;">
      <p style="font-size:32px;margin:0;">⭐⭐⭐⭐⭐</p>
      <p style="color:#6b7280;font-size:13px;margin:8px 0 0;">Répondez directement à cet email avec votre note et vos commentaires</p>
    </div>
    <p style="color:#374151;">Merci de votre confiance, et à bientôt sur VillaHub !</p>`)
}
...
exports.handler = async () => {
  ...
  return {
    statusCode: 200,
    body: JSON.stringify({ ok: true, sent, reminders: reminders?.length, welcomes: welcomes?.length, reviews: reviews?.length }),
  }
}
